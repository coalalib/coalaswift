...
extension SecurityLayer: OutLayer {
  ...
  func run(coala: Coala, message: inout CoAPMessage, toAddress: inout Address) throws {
    ...
    var proxySecurityId = getProxySecurityId(from: message)

    if message.proxyViaAddress != nil {

      if proxySecurityId == nil {
        proxySecurityId = arc4random()
      }
  ...
  func performHandshake(coala: Coala,
    ...
    if let proxySecurityId = getProxySecurityId(from: causingMessage) {
      message.setOption(.proxySecurityId, value: proxySecurityId)
    }
  ...
  private func getProxySecurityId(from message: CoAPMessage) -> UInt32? {
    return message.getOptions(.proxySecurityId).map { UInt32(data: $0.data)}.first
  }
